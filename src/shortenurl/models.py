...
def stu_info_pre_save(sender, instance, *args, **kwargs):
	...
	if not instance.original_link.startswith('http'):
		instance.original_link = 'https://' + instance.original_link

pre_save.connect(stu_info_pre_save, sender = UrlInfo)


class TWMInfo(models.Model):
	temperature = 	models.IntegerField(null = True, blank = True)
	date_time	=	models.DateTimeField(auto_now = True, null = True, blank = True)

	def __str__(self):
		return f'{self.temperature} C -- {self.date_time.date()} -- {self.date_time.time()}'
